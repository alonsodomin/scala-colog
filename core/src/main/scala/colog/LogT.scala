...
abstract case class LogT[F[_], A, B] private[colog] (
  ...
  def local[AA](f: Logger[F, AA] => Logger[F, A])(implicit F: Applicative[F]): LogT[F, AA, B] =
    new LogT[F, AA, B](
      unwrap.local(logger => f(logger.mapK(LogT.algebra[F, AA])).lift[LogT[F, A, ?]])
    ) {}
...
private[colog] trait LogTFunctions {
  ...
  private[colog] def algebra[F[_]: Applicative, A](
      implicit M: Monoid[Logger[F, A]]
  ): LogT[F, A, ?] ~> F =
    new (LogT[F, A, ?] ~> F) {
...
private[colog] trait LogTEffectInstances0 extends LogTMtlInstances {
  ...
  implicit def logTLiftIO[F[_]: Applicative, A](implicit F: LiftIO[F]): LiftIO[LogT[F, A, ?]] =
    new LiftIO[LogT[F, A, ?]] {
...
private[colog] trait LogTMtlInstances0 extends DirectLogTInstances {
  ...
  implicit def logTApplicativeLayer[F[_], A](
      implicit F0: Applicative[F]
  ): ApplicativeLayer[LogT[F, A, ?], F] =
...
private[colog] trait DirectLogTInstances2 extends DirectLogTInstances1 {
  ...
  implicit def logTMonad[F[_], A](implicit F0: Monad[F]): Monad[LogT[F, A, ?]] =
    new LogTMonad[F, A] {
      val F: Monad[F] = F0
    }
...
private[colog] trait DirectLogTInstances1 extends DirectLogTInstances0 {
  ...
  implicit def logTApplicative[F[_], Msg](
      implicit F0: Applicative[F]
  ): Applicative[LogT[F, Msg, ?]] = new LogTApplicative[F, Msg] {
...
private[colog] trait LogTConcurrent[F[_], Msg]
    extends Concurrent[LogT[F, Msg, ?]] with LogTAsync[F, Msg] {
  ...
  def racePair[A, B](
      fa: LogT[F, Msg, A],
      fb: LogT[F, Msg, B]
  ): LogT[F, Msg, Either[(A, LogTFiber[B]), (LogTFiber[A], B)]] =
    LogT { logger =>
...
private[colog] trait LogTSync[F[_], Msg]
    extends Sync[LogT[F, Msg, ?]] with LogTMonadError[F, Msg, Throwable] {
...
private[colog] trait LogTMonadError[F[_], Msg, Err]
    extends MonadError[LogT[F, Msg, ?], Err] with LogTMonad[F, Msg]
...
private[colog] trait LogTMonad[F[_], Msg]
    extends Monad[LogT[F, Msg, ?]] with LogTApplicative[F, Msg] {
...
private[colog] trait LogTApplicativeError[F[_], Msg, Err]
    extends ApplicativeError[LogT[F, Msg, ?], Err] with LogTApplicative[F, Msg] {
  ...
  override def handleErrorWith[A](fa: LogT[F, Msg, A])(f: Err => LogT[F, Msg, A]): LogT[F, Msg, A] =
    LogT(
      logger =>
        F.handleErrorWith(fa.via(logger))(err => f(err).via(Logger.loggerMonoid[F, Msg].empty))
    )

}

private[colog] trait LogTApplicative[F[_], Msg]
    extends Applicative[LogT[F, Msg, ?]] with LogTApply[F, Msg] {
